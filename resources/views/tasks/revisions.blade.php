        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {!! app('menu')->handler('tasks.panel-buttons.revisions')->render('inline') !!}
                    <h4>
                        {{$duty->name}}: {{trans('tasks.view_revisions', ['name' => $task->name])}}
                    </h4>
                </div>
                <div class="panel-body">

                    <dl class="timeline">
                    @foreach($task->revisionHistory->reverse() as $history)
                        @if($history->key == 'created_at' && !$history->old_value)
                            ...
                            <dd>{{trans('tasks.created_this')}}</dd>
                        @else
                            ...
                            <dd>{{trans('tasks.changed_from_to', [
