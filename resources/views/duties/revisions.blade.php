        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {!! app('menu')->handler('duties.panel-buttons.revisions')->render('inline') !!}
                    <h4>
                        {{$occupation->name}}: {{trans('duties.view_revisions', ['name' => $duty->name])}}
                    </h4>
                </div>
                <div class="panel-body">

                    <dl class="timeline">
                    @foreach($duty->revisionHistory->reverse() as $history)
                        @if($history->key == 'created_at' && !$history->old_value)
                            ...
                            <dd>{{trans('duties.created_this')}}</dd>
                        @else
                            ...
                            <dd>{{trans('duties.changed_from_to', [
